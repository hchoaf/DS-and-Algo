class Max_Heap_Tree():
    ...
    def sift_down(self, index):
        ...
        if l_index < len(self.heap_list) and self.heap_list[l_index] < self.heap_list[min_index]:
            min_index = l_index
        r_index = self.return_right_child(index)
        if r_index < len(self.heap_list) and self.heap_list[r_index] < self.heap_list[min_index]:
            min_index = r_index
        
        if index != min_index:
            self.heap_list[index], self.heap_list[min_index] = self.heap_list[min_index], self.heap_list[index]
...
def heap_sort_ascending(Array):
    ...
    for i in range(len(Array)):
        Array[i] = new_heap.extract_min()
    return Array

a = [10, 3, 6, 9, 2, 1, 4, 32]

print(heap_sort_ascending(a))
